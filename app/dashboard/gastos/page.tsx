  ...
  return (
    <div className="p-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Gastos</h1>
          <p className="text-slate-500 mt-1">Registro de gastos operativos</p>
        </div>
        <div className="flex items-center gap-4">
          <div className="text-right">
            <p className="text-xs text-slate-500">Total filtrado</p>
            <p className="text-xl font-bold text-slate-900 tracking-tight">L. {totalGastos.toLocaleString()}</p>
          </div>
          <button
            onClick={() => setShowModal(true)}
            className="flex items-center gap-2 bg-brand-primary hover:bg-brand-primary-dark text-white px-6 py-3 rounded-2xl font-medium transition-colors shadow-sm shadow-brand-primary/25"
          >
            <Plus size={20} />
            Nuevo Gasto
          </button>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        ...
        <select
          ...
          className="px-4 py-3 border border-slate-200 rounded-2xl focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary text-sm bg-white transition"
        >
            ...
            <tbody>
              {loading ? (
                ...
                gastosFiltrados.map((gasto: any) => (
                  <tr key={gasto.id} className="border-t border-slate-100 hover:bg-slate-50/80 transition-colors">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl">
            <div className="flex items-center justify-between px-8 pt-8 pb-4">
              <h3 className="text-2xl font-bold text-slate-900 tracking-tight">Nuevo Gasto</h3>
            ...
            <form onSubmit={crearGasto} className="px-8 pb-8 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select name="categoria" className="w-full border border-slate-200 rounded-2xl px-4 py-3 text-base focus:outline-none focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary bg-white transition">
                    {CATEGORIAS.map(cat => (
                      <option key={cat} value={cat}>{cat}</option>
                    ))}
                  </select>
              ...
              <div className="flex gap-3 pt-4">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3.5 rounded-2xl border border-slate-200 text-slate-700 font-medium hover:bg-slate-50 transition-colors">
                  Cancelar
                </button>
                <button type="submit" className="flex-1 py-3.5 rounded-2xl bg-brand-primary hover:bg-brand-primary-dark text-white font-semibold transition-colors shadow-sm shadow-brand-primary/25">
                  Guardar Gasto
                </button>
              </div>
            </form>
